...
void GLTFMetallic_Roughness::build_pipelines(VulkanEngine *engine) {
  ...
  if (!vkutil::load_shader_module("./shaders/mesh.frag.spv", engine->_device,
                                  &meshFragShader)) {
    fmt::println("Error when building triangle fragment shader module");
  }

  VkShaderModule meshVertexShader;
  if (!vkutil::load_shader_module("./shaders/mesh.vert.spv", engine->_device,
                                  &meshVertexShader)) {
    fmt::println("Error when building triangle vertex shader module");
  }
...
AllocatedImage VulkanEngine::create_image(VkExtent3D size, VkFormat format,
  ...
  if (mipmapped) {
    img_info.mipLevels =
        static_cast<uint32_t>(floor(log2(glm::max(size.width, size.height)))) +
        1;
  }
...
void VulkanEngine::init_default_data() {
  ...
  GLTFMetallic_Roughness::MaterialConstants *sceneIniformData = {
      (GLTFMetallic_Roughness::MaterialConstants *)
          materialConstants.allocation->GetMappedData()};
  sceneIniformData->colorFactors = glm::vec4{1, 1, 1, 1};
  sceneIniformData->metal_rough_factores = glm::vec4{1, 0.5, 0, 0};
